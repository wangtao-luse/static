...
(function($) {
	function show(el, type, _258, _259) {
		...
		var temIndex = $.fn.window.defaults.zIndex;
		if($(".success-window-mask").length > 0) {
			$(".success-window-mask").remove()
		} else {
			win.after("<div class=\"success-window-mask\"></div>");
			$(".success-window-mask").css("z-index", temIndex - 2)
		}
		if(!win) {
			return
		}
						...
						if(type == "footer") {
							...
							if(type == "selected") {
								...
								if(type == "highlight") {
									...
									if(type == "checked") {
										return(_780 == 1 ? dc.body1 : dc.body2).find(">table>tbody>tr.datagrid-row>td>div.datagrid-cell-check>input:checked").parent().parent().parent()
									} else {
										if(type == "last") {
	...
	function _852(_853) {
		...
		function _855() {
			...
			_858.find(".datebox-current").click(function() {
				...
				if($(this).parent().siblings().find(".timespinner-f").length) {} else {
					_85b(_853, opts.formatter(new Date()));
					$(_853).combo("hidePanel")
				}
			});
